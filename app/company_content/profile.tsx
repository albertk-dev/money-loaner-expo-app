...
import { View, Text,TouchableOpacity, StyleSheet, TextStyle, Alert, Image, ScrollView, TextInputProps, FlatList, ActivityIndicator } from 'react-native';
...
interface CompanyUpdatable extends Omit<ICompany,'createdAt' | 'logoCloudImagePath' | 'updatedAt' | "employees">{
...
const colors = useAppThemeColor()
  ...
  const updatingCompanySuccess = useSelector(state => state.company.updateCompanySuccess);
  const updatingCompanyErrorMessage = useSelector(state => state.company.errorUpdatingCompany);
  ...
  const { control, handleSubmit, formState: { errors}, setValue, clearErrors,  reset } = useForm<CompanyUpdatable>({
    defaultValues: {
      name: company.name,
    ...
    const companyProfileItems:Array<ProfileField> = [
      {
        name: 'name',
        label: 'Raison social',
        helperText:"il s'agit du nom de votre entreprise celui que tout le monde connait",
        required: true,
      ...
      },
      {
        ...
        canEdit: false,
        required:false,
        helperText: "il s'agit de votre mot de passe, vous ne pouvez ni la voir ni la modifier dans ce formulaire clisuez sur le bouton de modification pour lancer la procédure",
        ...
        endIconProps: { width: 32, height: 32, fill: colors.primary, fillOpacity: 1 },
         handleClicEndIcon: () => Alert.alert("modification du mot de passe lancer..."),
      ...
      },
      {
        ...
        canEdit: true,
        required:true,
        helperText: "il peut s'agir d'un matricule ou d'un numéro d'arrété...toutes choses qui identifie de facon unique votre entreprise au yeux de l'état ",
      ...
      },
      {
        ...
        helperText: "il s'agit du numéro de téléphone de votre entreprise, il vous sera proposer par défaut lors des rembourssement",
        startIcon: APP_IMAGES.ICON_PHONE_NUMBER_FIXE_FIELD,
        textInputProps:{keyboardType:'phone-pad',autoCapitalize:'none', autoCorrect:false}
      },
      {
        name: 'email',
        label: 'Email',
      ...
      },
      {
        ...
        canEdit: false,
        required:false,
        helperText: "il s'agit du nombre d'employées que vous avez ajouté a l'application ",
    ...
    () => {
      ...
      setIsOperationInProgress(false)
        getDownloadURL(uploadTask.snapshot.ref).then((url)=>{
            ...
            const data: UploadToFirebaseCompleted = { logoCloudImagePath: company.logoCloudImagePath, logoURL: url }
          return data;
        });
    }
  );
...
const onUpdate = async (data: CompanyUpdatable) => {
  ...
  return (
    <SafeAreaView style={{...style.page, padding:0,margin:0, paddingVertical:0, paddingHorizontal:0}} >
      ...
      <View style={{ flexDirection: 'row', width: '100%', justifyContent: 'space-between', alignItems: 'center', backgroundColor: colors.background, borderRadius: 50 }}>
        ...
        <ScrollView contentContainerStyle={{ gap: 20, paddingTop:10 }} showsVerticalScrollIndicator={false} >
          ...
          <Controller
                        ...
                        render={({ field: { onChange, onBlur, value } }) => (
                            <View style={{ ...style.flexCenter, marginBottom: 2, width: '100%', padding: 15, position: 'relative', borderWidth: 1, borderRadius: 10, borderColor: (errors.logoURL ? colors.danger : 'transparent') }}>
                                 <View style={{alignItems:'center', justifyContent:'space-between',width:'100%', flexDirection:'row', gap:20}}>
            ...
            <View style={{backgroundColor: !editingProfile? Color(colors.primary).alpha(0.5).toString() :colors.primary, borderRadius:10,flexDirection:'row-reverse', padding:10,gap:10, alignItems:'center'}} >
              <Text style={{...fonts.bodyHighLight, color:colors.white} as TextStyle}>Modifier le logo</Text>
              <APP_IMAGES.ICON_CLOUD  width={32} height={32} fill={colors.white}/>
            </View>
                              </TouchableOpacity> : <Text>logo</Text>} 
            <Image source={{uri: value}} width={64} height={64}/>
          </View>

                                {errors.logoURL && (
                                    <TouchableOpacity onPress={() => showAlert(errors?.logoURL?.message!)} style={{ backgroundColor: colors.danger, position: "absolute", right: 0, top: 0, borderTopRightRadius: 10, borderBottomLeftRadius: 10 }}>
          ...
          <View>
            {updatingCompanySuccess == false && <Text>{updatingCompanyErrorMessage}</Text>}
          </View>


          
          {editingProfile && <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
            ...
            >
              <View style={{ backgroundColor: updatingCompany===true || isOperationInProgress? colors.background: colors.white, width: 113, height: 82, borderRadius: 10, padding: 10, gap: 10, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: colors.primary }} >
            ...
            <TouchableOpacity onPress={handleSubmit(onUpdate)} disabled={updatingCompany! || isOperationInProgress}>
              <View style={{ backgroundColor: updatingCompany===true || isOperationInProgress ? colors.white : colors.primary, width: 113, height: 82, borderRadius: 10, padding: 10, gap: 10, justifyContent: 'center', alignItems: 'center' }} >
               { updatingCompany===true || isOperationInProgress ? <ActivityIndicator size={'large'} color={colors.primary}/> : <View><APP_IMAGES.ICON_PEN fill={colors.white} />
