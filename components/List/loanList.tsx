...
import { View, FlatList, StyleSheet, TextStyle, Text, Alert } from 'react-native';
...
const LoanList: React.FC<LoanListProps> = ({
  ...
  const renderItem = ({ item }: { item: ILoan }) => (
    <LoanItem 
      onRepay={onRepay} 
      readyToSelect={selectedLoans.length !== 0} 
      entityType={entityType} 
      menuItems={menuItems} 
      loan={item} 
      onSelect={(selected, data) => {
        setSelectedLoans(prevSelectedLoans => {
          const updatedLoans = selected 
            ? [...prevSelectedLoans, data] 
            : prevSelectedLoans.filter(employee => employee._id !== data._id);
